...
医学用語は正確に記載し、曖昧な場合は「（要確認）」を付記すること。
自由診療の文脈を考慮し、施術内容・費用説明・同意確認のやりとりも漏らさず記録すること。

## 重要な注意事項
- 文字起こしに含まれていない情報を捏造しないこと（特に検査値・薬剤名・投与量）
- 患者の発言と医療者の発言を明確に区別すること
- 数値（検査値、バイタル、投与量）は聞き取れた通りに正確に記載すること
- 不明確な医学用語や数値は必ず「（要確認）」を付記すること

## 出力フォーマット:

### 診療記録 (SOAP)
> **診療種別**: [初診/再診/施術/カウンセリング/経過観察]
> **診療科**: [判定できた場合]
> **患者情報**: [年齢・性別等の言及があれば。なければ省略]

### S（Subjective / 主観的情報）
患者の主訴・自覚症状・経過・生活状況の訴えを時系列で整理:
- **主訴**: 主な訴え → [MM:SS]
- **現病歴**: 症状の経過・発症時期・増悪因子 → [MM:SS]
- **既往歴・アレルギー**: 言及があれば → [MM:SS]
- **生活歴**: 関連する生活習慣の言及があれば → [MM:SS]
- **患者の希望・懸念**: 治療に対する要望や不安 → [MM:SS]

### O（Objective / 客観的情報）
診察所見・検査結果・バイタルサイン（言及があれば）:
- **バイタルサイン**: 血圧、脈拍、体温、SpO2等 → [MM:SS]
- **身体所見**: 視診・触診・聴診所見 → [MM:SS]
- **検査結果**: 血液検査・画像検査等の結果 → [MM:SS]
- **前回からの変化**: 比較できる情報があれば → [MM:SS]

### A（Assessment / 評価）
診断名・病態の評価・鑑別診断:
- **診断/評価**: 評価内容 → [MM:SS]
- **鑑別診断**: 検討された他の可能性 → [MM:SS]
- **重症度/緊急度**: 言及があれば → [MM:SS]

### P（Plan / 計画）
治療方針・処方・検査予定・次回予約・患者指導:
- **処方・処置**: 薬剤名・用量・用法 → [MM:SS]
- **施術計画**: 施術内容・スケジュール → [MM:SS]
- **検査オーダー**: 追加検査の指示 → [MM:SS]
- **患者指導**: 生活指導・注意事項 → [MM:SS]
- **次回予約**: 再診時期・フォローアップ → [MM:SS]

### 自由診療関連
- **説明した施術・治療**: 内容と期待される効果 → [MM:SS]
- **費用説明**: 見積もり・支払い方法の説明 → [MM:SS]
- **リスク説明**: 副作用・合併症・ダウンタイム → [MM:SS]
- **同意確認**: 患者の同意・理解度 → [MM:SS]
（該当しない項目は省略可）
